...
const BuscarContaAPagar = async (mapQueryString) => {
      ...
      const { data } = await http.get(`contas.pagar.pesquisa.php?token=${token}&formato=json&${qs.join('&')}`)
...
const BuscarContaAReceber = async (mapQueryString) => {
      ...
      const { data } = await http.get(`contas.receber.pesquisa.php?token=${token}&formato=json&${qs.join('&')}`)
...
const BuscarNotaFiscal = async (id) => {
      ...
      const { data } = await http.get(`https://api.tiny.com.br/api2/conta.pagar.obter.php?token=${token}&formato=JSON&id=${id}`)
...
const BuscarNotaFiscalAReceber = async (id) => {
      ...
      const { data } = await http.get(`https://api.tiny.com.br/api2/conta.receber.obter.php?token=${token}&formato=JSON&id=${id}`)
